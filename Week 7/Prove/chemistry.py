...
Brennon Laney
06/12/21
Assignment: Calculate the molar mass of atoms
'''
...
def main():
    ...
    mass_g = float(input('What is the mass of is substance in grams? '))
    ...
    correct = symbol_quantity_list[0][0]
    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    molar_mass = compute_molar_mass(symbol_quantity_list, table)
    get_elements(elements, table, symbol_quantity_list)
    ...
    print(f'The molar mass is {molar_mass:.4f} g/mol')

    # Print the number of moles.
    print(f'There are {moles:.4f} moles')

    print('The elements in this chemical are:')
...
def make_periodic_table():
    '''
    What this does is it takes no parameters but it returns the 
    table below
    '''
    periodic_table = {
        'Ac':["Actinium", 227],
...
def compute_molar_mass(symbol_quantity_list_new, periodic_table):
    ...
    atomic_mass = 0
    total_atomic_mass = 0
    for i in range(0, len(symbol_quantity_list_new)):
        for j in periodic_table:
            
            if symbol_quantity_list_new[i][0] == j:
                atomic_mass = periodic_table[j][1] *  symbol_quantity_list_new[i][1]

            else:
                atomic_mass = 0


            total_atomic_mass = total_atomic_mass + atomic_mass 
...
def get_elements(elements, periodic_table, symbol_quantity_list):
    '''
    Parameters
        symbol_quantity_list -- This is a list that contains the elements 
        and the amount of that element
        elements -- is the list that the element names will be appeneded to
        periodic_table -- This is the periodic table

    This will loop through both the periodic table and the symbol_quantity_list
    to find matches and if it does it will append the name of that element to
    the list elements.
    ...
    for i in range(0, len(symbol_quantity_list)):
        for element in periodic_table:
            if symbol_quantity_list[i][0] == element:
                elements.append(periodic_table[element][0])
