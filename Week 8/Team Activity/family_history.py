...
def print_death_age(people_dict):
    ...
    for i in people_dict:
        death_age = people_dict[i][3] - people_dict[i][2]
        print(f'{people_dict[i][0]} died at age {death_age}')
        print(f'{people_dict[i][2]} - {people_dict[i][3]}')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for i in marriages_dict:
        husband = people_dict[marriages_dict[i][0]][0]
        wife = people_dict[marriages_dict[i][1]][0]
        year = marriages_dict[i][2]
        age_husband = marriages_dict[i][2] - people_dict[marriages_dict[i][0]][2]
        age_wife = marriages_dict[i][2] - people_dict[marriages_dict[i][1]][2]

        print(f'{husband.capitalize()}, at age {age_husband}, married {wife.capitalize()} who was {age_wife}')
...
def count_marriage(marriages_dict, people_dict):
    '''
    This takes the parameters of the dictionaries marriages_dict
    and people_dict. It will go through a loop of the people in
    in the people_dict and then for each person it will loop through 
    the marriages_dic and find the people's id and count the amount 
    of times it appears in the marriage dictionary
    ...
    # It will loop through the people_dict
    for i in people_dict:

        # For every person it will loop through the marriage_dict
        #to find if the persons id appears in the dictionary.
        #If it does then it adds it to the variable marriages.
        for j in marriages_dict:
            if i == marriages_dict[j][0] or i == marriages_dict[j][1]:
                marriages += 1

        print(f'{people_dict[i][0]} married {marriages} times')

        # This resets the marriages vairable so it doesn't keep
        #counting after it has counted it for one person
        marriages = 0
